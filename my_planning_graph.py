...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        :param level: int
            corresponds to a particular state in the PG
        ...
        for action in self.all_actions: # TODO: does this work?
            # if all an actions prenodes are in the corresponding state level, add action to
            # the graph
            if PgNode_a(action).prenodes.issubset(self.s_levels[level]):
        ...
        # for given level, check that each state has children and each action has parents

        # find child of PgNode_s(Have(Cake), True)


        for action in self.a_levels[level]:
            assert action != set(), 'empty set in level {}\n {}'. \
                format(level, self.a_levels[levels])

            assert action.parents, \
            "{} isn't a complete PG_a.\n action:{}\n parents:{}\n ".\
            format(action, action.action, action.parents, action.children)


        for state in self.s_levels[level]:
            assert state != set(), 'empty set in level {}\n {}'. \
                format(level, self.s_levels[levels])

            assert state.children, \
                   "{} isn't a complete PG_s.\n state:{}\n children:{}".\
                    format(state, (state.symbol, state.is_pos), state.children)
    ...
    def add_literal_level(self, level):
        ...
        self.s_levels.append(set())  # initialized empty
        ...
        for state in self.s_levels[level]:
            ...
            assert state.parents, "{} doesn't have parents.\n parents: {}\n " \
                                  "children: {}".format((state.symbol, state.is_pos),
                                                        state.parents, state.children)
        for action in self.a_levels[level-1]:
            assert action != set()

            assert action.children, "{} doesn't have children.\n parents: {}\n " \
                                    "children: {}".format((action.action.name,
                                                           action.action.args),
                                                          action.parents, action.children)

    def update_a_mutex(self, nodeset):
        """ Determine and update sibling mutual exclusion for A-level nodes

        Mutex action tests section from 3rd Ed. 10.3 or 2nd Ed. 11.4
        A mutex relation holds between two actions on a given level
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # check if a1 negation node is an added node for a2
        for rem in node_a1.action.effect_rem:
            if rem in node_a2.action.effect_add:
                return True
        # check if a1 node is a negation node for a2
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Does node1 add an effect node2 negates in its precond?

        for add in node_a1.action.effect_add:
            if add in node_a2.action.precond_neg:
                return True
        # Does node1 rem an effect node2 has in its precond?
        for rem in node_a1.action.effect_rem:
            if rem in node_a2.action.precond_pos:
                return True
        for add in node_a2.action.effect_add:
            if add in node_a1.action.precond_neg:
                return True
        # Does node1 rem an effect node2 has in its precond?
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # pairs of parents for 2 action nodes
        parents1 = node_a1.parents
        parents2 = node_a2.parents
        assert len(parents1) != 0, 'empty set in parents yo. \n parents1:{}\n ' \
                                             'parents2:{}\n node_a1:{}\n node_a2:{}\n node1Act:{}'.\
                                             format(parents1, parents2, node_a1, node_a2, (node_a1.action.name, node_a1.action.args))
        assert len(parents2) != 0, 'empty set in parents. \n parents1:{}\n ' \
                                             'parents2:{}\n node_a1:{}\n node_a2:{} \n'.\
                                             format(parents1, parents2, node_a1, node_a2)
        combs = combinations([parents1, parents2], 2)
        pairs=[]

        # DS transformation
        for pair in combs:

            app = tuple(sib.pop() for sib in pair)
            pairs.append(app)


        # If any pair is complementary or has inconsistent support, the nodes are mutex.
        for pair in pairs:
            if (self.negation_mutex(pair[0], pair[1]) or
                self.inconsistent_support_mutex(pair[0], pair[1]) ):
                return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # if not node_s1.parents or not node_s2.parents: return negation(x)
        #
        # preActPairs = combinations([node_s1.parents, node_s2.parents], 2)
        #
        # for pair in preActPairs:
        #     if not pair[0].is_mutex(pair[1]):
        #         return False
        #
        # return True
        ...
        for par1 in parents1:
            assert par1, 'empty parent1:{}'.format(par1)
        for par2 in parents2:
            assert par2, 'empty parent2:{}'.format(par2)
        #pdb.set_trace()
        #if not parents1 or not parents2: return False
        ...
        # If a single pair is not mutex, return False
        for pair in pairs:
            if not pair[0].is_mutex(pair[1]):
                return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for each in goals:
            for level in self.s_levels:
                if PgNode_s(each, True) in level:
